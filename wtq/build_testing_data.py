...
from prompts.prompt_wtq import PROMPT_CLS_TEST, PROMPT_VERIF_TEST
...
def generate_cls_prompt(id_table, prompt, table, inst_1, ans_1, sota_info) -> str:
    table_id = table[1]
...
if __name__ == '__main__':
    ...
    sota_combine = get_sota_data(test_id, test_dataset, sota_test)
    sota_inst = []
    ...
    file_path = "./testing_data/final_res.txt"
    final_res = read_list_from_file(file_path)
    file_path = "./testing_data/instruction.txt"
    instruction = read_list_from_file(file_path)
    prompt_cls_l = []
    for i in range(len(test_dataset)):
        table_info = test_dataset[i]
        ins_1 = instruction[i]
        ans_1 = final_res[i][0]
        sota_info = sota_inst[i]
        prompt_res = generate_cls_prompt(id_table, PROMPT_CLS_TEST, table_info, ins_1, ans_1, sota_info)
    ...
    for i in range(len(test_dataset)):
        ...
        ans_1 = final_res[i][0]
        sota_info = sota_inst[i]
        prompt_res = generate_cls_prompt(id_table, PROMPT_VERIF_TEST, table_info, ins_1, ans_1, sota_info)
